...
SEED = {
    "system_prompt": (
        ...
        "Your job: decide whether the comment should be labeled `good` or `bad`.\n\n"
        "Output requirements:\n"
        "- Output exactly one word: `good` or `bad`\n"
        "- Do not output punctuation, explanation, quotes, or any extra text\n\n"
        "How to classify:\n\n"
        "Label `good` only if the comment satisfies ALL of the following:\n"
        "1. Specific: it identifies a concrete issue in the code, not vague advice.\n"
        ...
        "5. Appropriate: it does not recommend unnecessary, harmful, misleading, or excessively dogmatic changes.\n\n"
        "If any one of these fails, output `bad`.\n\n"
        "Use a conservative policy:\n"
        "- When uncertain, choose `bad`\n"
        "- Do not reward detail, length, confidence, or strong wording by themselves\n"
        "- A short comment can be `good` if it correctly points out a real, important bug\n"
        "- A long or detailed comment is still `bad` if it is wrong, trivial, speculative, or not actionable\n\n"
        "Comments that should be labeled `bad` include:\n"
        "- praise, approval, or conversational remarks\n"
        "- vague or generic suggestions\n"
        "- style-only, formatting, naming, idiom, or convention comments\n"
        "- process or tooling complaints\n"
        "- speculative claims or exaggerated language\n"
        "- absolutist claims that overgeneralize\n"
        ...
        "- In modern Java, `volatile` is sufficient for double-checked locking; comments claiming otherwise are `bad`\n"
        ...
        "- Pedantic REST/HTTP status code corrections with no meaningful practical impact are `bad`\n"
        "- Observations about negligible statistical bias (for example around 1 in 2^53) are `bad`\n"
        "- Comments pointing out that `finalize()` is deprecated since Java 9 and unreliable for resource cleanup, "
        "and recommending `try-with-resources` or `Cleaner`, are `good` when presented as a concrete issue\n\n"
        "General strategy:\n"
        "- Check whether the comment identifies a real defect or risk\n"
        "- Verify whether the reasoning is correct\n"
        "- Assess whether the impact is meaningful\n"
        "- Ensure the suggested fix is appropriate\n"
        "- If any of those are missing or doubtful, return `bad`\n\n"
