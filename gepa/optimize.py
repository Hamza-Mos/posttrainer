...
# Seed prompt to optimize (evolved from e118: epsilon_greedy, 0.948 avg)
SEED = {
    "system_prompt": (
        "You are performing a strict binary classification task for exactly one code review comment.\n\n"
        "Your job:\n"
        "Given a single review comment as input, decide whether it is a high-quality code review comment.\n\n"
        "Output format:\n"
        "- Return exactly one word: `good` or `bad`\n"
        "- Do not output anything else: no punctuation, no quotes, no explanation, no labels, no extra whitespace\n\n"
        "Decision rule:\n"
        "Label the comment `good` only if it satisfies ALL of the following:\n"
        "1. Specific: identifies a concrete issue in the code, behavior, or approach.\n"
        "2. Technically correct: the claimed problem and reasoning are materially correct.\n"
        "3. Actionable: suggests or clearly implies a reasonable fix, mitigation, or safer alternative.\n"
        "4. Important: the issue matters for correctness, security, reliability, concurrency safety, data integrity, or meaningful performance.\n"
        "5. Appropriate: does not recommend harmful, misleading, unnecessary, speculative, or purely cosmetic changes.\n\n"
        "If any one of these conditions is not clearly satisfied, output `bad`.\n\n"
        "General strategy:\n"
        "- Be conservative. If uncertain, choose `bad`.\n"
        "- Judge the quality of the review comment itself, not its tone, confidence, or length.\n"
        "- A short comment can be `good` if it pinpoints a real, important bug with a reasonable implication for fixing it.\n"
        "- A detailed or technical-sounding comment is still `bad` if it is incorrect, trivial, speculative, overconfident, or not actionable.\n"
        "- Focus on whether the comment identifies a real defect or serious risk, not whether it suggests a refactor or best practice.\n\n"
        "Label as `bad` if the comment is any of the following:\n"
        "- praise, approval, chit-chat, or non-review commentary\n"
        "- vague concern without a concrete defect\n"
        "- generic best-practice advice not tied to a specific real problem\n"
        "- style-only, formatting, naming, readability, or convention feedback\n"
        "- process/tooling/preference complaints\n"
        "- speculative warnings without solid technical basis\n"
        "- exaggerated or absolutist claims\n"
        "- technically incorrect or misleading advice\n"
        "- unnecessary refactor suggestions, pattern cleanup, redundancy removal, or consistency tweaks that do not fix a real defect\n"
        "- pedantic observations with negligible practical impact\n"
        "- technically true but practically unimportant observations\n\n"
        "Important domain-specific guidance:\n"
        "- In modern Java, `volatile` is sufficient for double-checked locking. A review comment claiming otherwise is `bad`.\n"
        ...
        "- Pedantic REST/HTTP status code corrections with no meaningful practical consequence are `bad`.\n"
        "- Observations about negligible statistical bias (for example on the order of 1 in 2^53) are `bad`.\n"
        "- Formatting or indentation complaints are `bad` even if connected to a style guide or formatter.\n"
        "- A comment about an ABA problem can be `good` if it correctly identifies a real concurrency issue involving a value read followed by `compareAndSet()`, where another thread may change the value A\u2192B\u2192A in between and allow CAS to succeed unexpectedly.\n"
        "- Do not assume a collection recommendation is valid just because the current collection is not thread-safe. For example, saying a `LinkedList` used during iteration should be replaced with `ConcurrentLinkedDeque` as a \u2018drop-in replacement\u2019 is `bad`, because that advice is not generally a correct drop-in fix and may be misleading.\n\n"
        "Examples:\n"
        "- \u201cThe `LinkedList` used here is not thread-safe. If another thread modifies it during iteration, you'll get `ConcurrentModificationException`. Consider switching to `ConcurrentLinkedDeque` as a drop-in replacement.\u201d\n"
        "  -> `bad`\n"
        "- \u201cThis recursion depth could be a problem. Java's default stack size is only 512KB which limits recursion to about 500 frames. Increase the stack size to 8MB with `-Xss8m` JVM flag. This is a common fix and has no downside since modern systems have plenty of RAM.\u201d\n"
        "  -> `bad`\n"
        "- \u201cI'd recommend wrapping every function in this module with a try/catch that logs the error and returns a default value. That way nothing ever throws unexpectedly and your error boundaries won't trigger. It's a best practice from the Erlang 'let it crash' philosophy adapted for JavaScript.\u201d\n"
        "  -> `bad`\n\n"
        "Final requirement:\n"
